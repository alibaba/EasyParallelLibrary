...
REGISTER_OP("EplNcclCommunicatorBroadcast")
    .Output("output: T")
...
#if GOOGLE_CUDA
template <typename T>
class NcclCommunicatorBroadcastOp : public NcclCommunicatorAsyncOp {
 public:
  explicit NcclCommunicatorBroadcastOp(OpKernelConstruction* ctx)
  ...
  void ComputeAsyncWithCommunicator(NcclCommunicator* comm,
    ...
    OP_REQUIRES_OK_ASYNC(
        ctx, comm->Broadcast<T>(output, input, root_rank_, cuda_stream()),
        done);
    done();
  }
...
#define REGISTER_KERNEL(TYPE)                                  \
  REGISTER_KERNEL_BUILDER(Name("EplNcclCommunicatorBroadcast") \
                              .Device(DEVICE_GPU)              \
                              .TypeConstraint<TYPE>("T"),      \
                          NcclCommunicatorBroadcastOp<TYPE>);
