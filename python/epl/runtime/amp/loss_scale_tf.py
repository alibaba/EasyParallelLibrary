...
Code modified based on https://github.com/tensorflow/tensorflow/blob/r1.15/tensorflow/python/training/experimental/loss_scale.py"""
...
is_112 = Version(__version__) < Version("1.15.0")
if is_112:
  from tensorflow.python.training.checkpointable.tracking import base
  BaseClass = base.CheckpointableBase # pylint: disable=invalid-name
else:
  from tensorflow.python.training.tracking import base as trackable
  BaseClass = trackable.Trackable # pylint: disable=invalid-name

@six.add_metaclass(abc.ABCMeta)
@tf_export('train.experimental.LossScale')
class LossScale(BaseClass):
  ...
  def _add_weight(self, name, initial_value, dtype=None):
    ...
    if is_112:
      self._handle_deferred_dependencies(name=name, checkpointable=variable)
    else:
      self._handle_deferred_dependencies(name=name, trackable=variable) # pylint: disable=unexpected-keyword-arg,no-value-for-parameter
    return variable
...
class DynamicLossScale(LossScale):
  ...
  def update(self, grads):
    ...
    # TODO(sayang): check nan for all replicas
