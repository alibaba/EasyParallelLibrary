...
class StrategyTest(test.TestCase):
  ...
  def test_pipeline_with_dp(self):
    config = epl.Config()
    config.pipeline.num_micro_batch = 4
    epl.init(config)
    with tf.Graph().as_default():
      ...
      self.assertEqual(g.num_stages, 2)
      self.assertEqual(g.get_pipeline_config().num_micro_batch, 4)
      for i in range(2):
        self.assertEqual(g.taskgraphs[i].local_num_replicas, 2)
  ...
  def test_pipeline(self):
    ...
    with tf.Graph().as_default():
      ...
      with epl.replicate(name="stage_1", device_count=1):
        x = tf.layers.dense(inputs=x, units=16, activation=None)
      with epl.replicate(name="stage_2", device_count=1):
        x = tf.layers.dense(inputs=x, units=16, activation=None)
      with epl.replicate(name="stage_3", device_count=1):
        x = tf.layers.dense(inputs=x, units=16, activation=None)
        logits = tf.layers.dense(inputs=x, units=10, activation=None)
        loss = tf.reduce_mean(logits)
      g = Graph.get()
      self.assertEqual(g._current_model_phase, ModelPhase.FORWARD)
      optimizer = tf.train.AdamOptimizer(learning_rate=0.01)
      ...
      self.assertTrue(Env.get().cluster is not None)
      self.assertEqual(Env.get().cluster.worker_num, 1)
      self.assertEqual(len(Env.get().cluster.virtual_devices), 4)
      self.assertEqual(Env.get().cluster.virtual_devices[0]._slice_devices,
                       [['/job:worker/replica:0/task:0/device:GPU:0']])
      self.assertEqual(Env.get().cluster.virtual_devices[1]._slice_devices,
                       [['/job:worker/replica:0/task:0/device:GPU:1']])
      self.assertEqual(Env.get().cluster.virtual_devices[2]._slice_devices,
                       [['/job:worker/replica:0/task:0/device:GPU:2']])
      self.assertEqual(Env.get().cluster.virtual_devices[3]._slice_devices,
                       [['/job:worker/replica:0/task:0/device:GPU:3']])
      self.assertEqual(g.num_stages, 4)
      self.assertEqual(g.get_pipeline_config().num_micro_batch, 4)
      for i in range(4):
        self.assertEqual(g.taskgraphs[i].local_num_replicas, 1)
      with tf.train.MonitoredTrainingSession() as sess:
        loss_value = sess.run([train_op, loss])
        print(loss_value)
  ...
  def test_dp(self):
    ...
    with tf.Graph().as_default():
      with epl.replicate(device_count=1):
        ...
        self.assertEqual(g.num_stages, 1)
        self.assertEqual(g.get_pipeline_config().num_micro_batch, 1)
        self.assertEqual(g.taskgraphs[0].local_num_replicas, 4)
        with tf.train.MonitoredTrainingSession() as sess:
          loss_value = sess.run([train_op, loss])
          print(loss_value)
